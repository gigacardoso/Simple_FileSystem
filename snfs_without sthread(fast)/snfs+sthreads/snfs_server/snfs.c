...
void snfs_remove(snfs_msg_req_t *req, int reqsz, snfs_msg_res_t *res, int* ressz)
{
   ...
   char* name = req->body.remove.name;
   ...
   *ressz = sizeof(*res) - sizeof(res->body) + sizeof(res->body.remove);
   ...
   name[MAX_FILE_NAME_SIZE-1] = '\0';
   inodeid_t fileid;
   if (!fs_remove(FS,dir,name, &fileid)) {
      res->status = RES_OK;
...
{
   // get input arguments
   inodeid_t src = (inodeid_t)req->body.copy.src_dir;
   inodeid_t dst = (inodeid_t)req->body.copy.dst_dir;
   ...
   *ressz = sizeof(*res) - sizeof(res->body) + sizeof(res->body.copy);
   ...
   inodeid_t fileid;
   if (!fs_copy(FS,src,src_name ,dst,dst_name, &fileid)){
			res->status = RES_OK;
...
{
	// get input arguments
   inodeid_t dir1 = (inodeid_t)req->body.append.dir1;
   inodeid_t dir2 = (inodeid_t)req->body.append.dir2;
   char* src_name = req->body.copy.src_name;
   char* dst_name = req->body.copy.dst_name;
   ...
   *ressz = sizeof(*res) - sizeof(res->body) + sizeof(res->body.append);
   ...
   src_name[MAX_FILE_NAME_SIZE-1] = '\0';
   dst_name[MAX_FILE_NAME_SIZE-1] = '\0';
   if (!fs_append(FS,dir2, src_name,dir1, dst_name)){
      fs_file_attrs_t attrs;
      if (fs_get_attrs(FS,dir1,&attrs) == 0) {
         res->status = RES_OK;
         res->body.append.fsize = attrs.size;
      }
   }
}	   
...
{
		// prepare the response
   *ressz = sizeof(*res) - sizeof(res->body) + sizeof(res->body.filesystem);
   res->type = REQ_DEFRAG;
   res->status = RES_ERROR;
